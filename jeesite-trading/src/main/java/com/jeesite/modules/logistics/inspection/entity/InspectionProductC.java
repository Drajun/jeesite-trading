...
@Table(name="reference_product_c", alias="a", columns={
		...
		@Column(name="reference_id", attrName="inspectionCId.id", label="托运单ID", isQuery=true),
		...
		@Column(name="hs_number", attrName="hsNumber", label="H.S.编码", isQuery=false),
	}, orderBy="a.create_date ASC"
)
public class InspectionProductC extends DataEntity<InspectionProductC> {
	
	private static final long serialVersionUID = 1L;
	private InspectionC inspectionCId;		// 托运单ID 父类
	...
	private String hsNumber;		// H.S.编码
	
	public InspectionProductC() {
		this(null);
	}


	public InspectionProductC(InspectionC consignCId){
		this.inspectionCId = consignCId;
	}
	
	public InspectionC getInspectionCId() {
		return inspectionCId;
	}

	public void setInspectionCId(InspectionC inspectionCId) {
		this.inspectionCId = inspectionCId;
	}
	...
	@NotNull(message="H.S.编码不能为空")
	@Length(min=0, max=64, message="H.S.编码长度不能超过 64 个字符")
